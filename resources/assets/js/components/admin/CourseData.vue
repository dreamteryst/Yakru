  ...
  <section>
    ...
    >
      ...
      <div class="table-responsive">
        <table class="table table-bordered">
          <tr>
            <th class="text-nowrap">ชื่อผู้เรียน</th>
            <th class="text-nowrap">คืนเงิน (บาท)</th>
            <th class="text-nowrap">วันที่ชำระ</th>
          </tr>
          <tr v-for="(order, i) in data.orders" :key="i">
            <td>{{ order.user.firstname }} {{ order.user.lastname }}</td>
            <td>{{ data.refund_percentage * order.course_price / 100 }}</td>
            <td>{{ order.created_at }}</td>
          </tr>
        </table>
        <div class="checkbox">
          <label>
            <input type="checkbox" v-model="refund"> ทำการคืนเงิน
          </label>
        </div>
      </div>
    </b-modal>
    ...
    >
      ...
      <div class="form-group">
        ...
        <select
          class="form-control"
          :class="{'is-invalid':isError('category_id')}"
          id="category_id"
        ...
        <div class="invalid-feedback" v-if="isError('category_id')">{{ errors.category_id[0] }}</div>
      </div>
      <div class="form-group">
        ...
        <input
          type="text"
          class="form-control"
          :class="{'is-invalid':isError('course_name')}"
          id="course_name"
          v-model="data.course_name"
          placeholder="Course Name"
        >
        <div class="invalid-feedback" v-if="isError('course_name')">{{ errors.course_name[0] }}</div>
      </div>
      <div class="form-group">
        ...
        <input
          ...
          class="form-control"
          id="course_subtitle"
          :class="{'is-invalid':isError('course_subtitle')}"
          v-model="data.course_subtitle"
          placeholder="Course Subtitle"
        >
        <div
          class="invalid-feedback"
          v-if="isError('course_subtitle')"
        >{{ errors.course_subtitle[0] }}</div>
      </div>
      <div class="form-group">
        ...
        <textarea
          ...
          :class="{'is-invalid':isError('course_description')}"
          v-model="data.course_description"
          placeholder="Course Description"
          rows="15"
        ></textarea>
        <div
          class="invalid-feedback"
          v-if="isError('course_description')"
        >{{ errors.course_description[0] }}</div>
      </div>
      <div class="form-group">
        ...
        <textarea
          class="form-control"
          id="refund_policy"
          :class="{'is-invalid':isError('refund_policy')}"
          v-model="data.refund_policy"
          placeholder="Course Refund Policy"
          rows="8"
        ></textarea>
        <div class="invalid-feedback" v-if="isError('refund_policy')">{{ errors.refund_policy[0] }}</div>
      </div>
      <div class="form-group">
        ...
        <input
          ...
          class="form-control"
          id="course_price"
          :class="{'is-invalid':isError('course_price')}"
          v-model="data.course_price"
          placeholder="Course Price"
        >
        <div class="invalid-feedback" v-if="isError('course_price')">{{ errors.course_price[0] }}</div>
      </div>
      <div class="form-group">
        ...
        <input
          ...
          :class="{'is-invalid':isError('course_discounted')}"
          v-model="data.course_discounted"
          placeholder="Course Price"
        >
        <div
          class="invalid-feedback"
          v-if="isError('course_discounted')"
        >{{ errors.course_discounted[0] }}</div>
      </div>
      <div class="form-group">
        ...
        <input
          ...
          :class="{'is-invalid':isError('refund_percentage')}"
          v-model="data.refund_percentage"
          placeholder="Course Refund Percentage"
        >
        <div
          class="invalid-feedback"
          v-if="isError('refund_percentage')"
        >{{ errors.refund_percentage[0] }}</div>
      </div>
      <div class="form-group">
        ...
        <input
          ...
          class="form-control"
          id="course_limit"
          :class="{'is-invalid':isError('course_limit')}"
          v-model="data.course_limit"
          placeholder="Course Price"
        >
        <div class="invalid-feedback" v-if="isError('course_limit')">{{ errors.course_limit[0] }}</div>
      </div>
      ...
      <div class="form-group">
        ...
        <input-tag
          ...
          class="form-control"
          id="tags"
          :class="{'is-invalid':isError('tags')}"
          :tags.sync="data.tags"
          placeholder="Tags"
          addTagOnBlur
          :addTagOnKeys="[13,188,9,32]"
        />
      </div>
    </b-modal>
  </section>
...
export default {
                ...
                columns: [
                    ...
                    { data: "fullname", name: "fullname" },
                    {
                        ...
                        render: (data, type, row, meta) => {
                            return `<a href="/admin/course/unit/${
                                row.id
                            }">${data}</a>`;
                        }
                    },
                    ...
                    },
                    {
                        ...
                        render: (data, type, row, meta) => {
                            return `<ul>
                                <li><a href="/admin/course/unit/${
                                    data.id
                                }">หลักสูตร</a></li>
                                <li><a href="/admin/course/example/${
                                    data.id
                                }">แบบทดสอบ</a></li>
                                ${
                                    row["type"] == "live"
                                        ? `<li><a href="/admin/course/live/${
                                              row["id"]
                                          }" style="white-space: nowrap;">ตารางสอน</a></li>`
                                        : ``
                                }
                                </ul>`;
                        },
                        ...
                        .each(function(group, i) {
                            if (last !== group) {
                                $(rows)
                                    .eq(i)
                                    .before(
                                        '<tr class="group"><td colspan="8">หมวดหมู่ ' +
                                            group +
                                            "</td></tr>"
                                    );
    ...
    methods: {
        ...
        editData(evt) {
            ...
            formData.append(
                "requirements",
                JSON.stringify(this.data.requirements)
            );
